...
import velum.client.IIIiiiii_Class32;
...
import velum.client.IiiIiIIii_Class212;
...
public class IIiIIiiI_Class39 {
    public IIIiiIIi_Class26 I_method_54df638b() {
        List<String> list = VelumClient.getInstance().getModuleManager().getModules().stream().map(iiIiiIii_Class92 -> iiIiiIii_Class92.getName().replace(" ", "")).toList();
    ...
    private void I_method_f910336d(IIIiiIii_Class28 iIIiiIii_Class28) {
        ...
        if (string.equalsIgnoreCase("clear")) {
            for (ModuleEntry iiIiiIii_Class922 : VelumClient.getInstance().getModuleManager().getModules()) {
