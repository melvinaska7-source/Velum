...
public class IiIiiIIII_Class177 extends IiIiIIiII_Class165 implements iIIiIIiIi_Class294, iIIiIIiiI_Class295 {
   ...
   public IiIiiIIII_Class177() {
      ...
      VelumClient.getInstance().I_method_7a4c3f0b().I_method_de9edf96("client");
   ...
   public void renderComponent(III var1) {
      ...
      this.renderLocalMusicIsland(var1);
   }

   /** Compact Velum music status shared by both ClickGUIs. */
   private void renderLocalMusicIsland(III draw) {
      MusicModule module = VelumClient.getInstance().getModuleManager().getModule(MusicModule.class);
      if (module == null) {
         return;
      }

      VelumLocalMusic music = VelumLocalMusic.getInstance();
      String title = module.isEnabled() && music.isPlaying() ? music.getCurrentTitle() : "Music Off";
      if (title.isEmpty()) {
         title = "Music Off";
      }
      title = title.length() > 30 ? title.substring(0, 27) + "..." : title;

      String time = module.isEnabled() && music.isPlaying() ? formatMusicTime(music.getPositionMs()) : "";
      float width = Math.max(92.0F, Math.min(220.0F, 32.0F + title.length() * 3.45F + (time.isEmpty() ? 0.0F : 28.0F)));
      float height = 18.0F;
      ...
      draw.drawText(IIiIiI_Class11.II_field_857c0621.I_method_3a2d5e3(7.0F), title, x + 8.0F, y + 5.0F, text.mulAlpha(0.9F));
      if (!time.isEmpty()) {
   ...
   public String I_method_fc79a821() {
      ...
      return var1 != null && !var1.isBlank() ? iIIIIiIiI_Class267.i_method_e7444577(var1.trim()) : "Velum";
   }
