...
public class IiIiiIIII_Class177 extends IiIiIIiII_Class165 implements iIIiIIiIi_Class294, iIIiIIiiI_Class295 {
   ...
   /** Small local-player island; intentionally compact so it never becomes a full HUD panel. */
   private void renderLocalMusicIsland(III draw) {
      ...
      title = title.length() > 34 ? title.substring(0, 31) + "..." : title;
      String time = formatMusicTime(music.getPositionMs());
      float width = Math.max(86.0F, Math.min(220.0F, 22.0F + title.length() * 3.45F + 26.0F));
      float height = 18.0F;
