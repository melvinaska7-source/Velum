...
import pyvelum.events.game.WorldChangeEvent;
...
implements iIIiIIiIi_Class294 {
    ...
    private final IiIIIiII_Class69<HudRenderEvent> handleGlobals = hudRenderEvent -> {
        RocknetMenu rocknetMenu = VelumClient.getInstance().I_method_449a4cbc();
        if (rocknetMenu == null) {
            VelumClient.getInstance().I_method_d0f1adb4(new RocknetMenu());
            return;
        }
    ...
    private void checkVisibilityChange() {
        GlobalsMenuModule iIIIIiiIi_Class14 = VelumClient.getInstance().getModuleManager().getModule(GlobalsMenuModule.class);
        ...
        if (!string.equals(this.lastVisibility)) {
            ...
            VelumClient.getInstance().I_method_cd3d46d0().updateVisibility(string);
        }
    }
    ...
    public static void send() {
        executor.submit(() -> {
            ...
            VelumClient.getInstance().I_method_cd3d46d0().info(I_field_3a9bda27.getSession().getUsername(), String.valueOf(worldKey.anarchy()), worldKey.server(), iIIIiiiII_Class285.i_method_be2f04a1(), worldKey.world(), string2, Profile.getUsername());
            ...
            if (!I_field_3a9bda27.isInSingleplayer() && string3 != null && !string3.equals("single")) {
                ...
                if (!(string == null || string.isBlank() || !bl && string.equals(lastJoinedServer))) {
                    ...
                    lastOnServerAt = System.currentTimeMillis();
                    VelumClient.getInstance().I_method_cd3d46d0().send(new Packets.Nested1_3422a40(string, iIIIiiiII_Class285.i_method_be2f04a1(), string3, I_field_3a9bda27.getSession().getUsername(), "Velum".toLowerCase()));
                }
            } else {
            ...
            VelumClient.getInstance().I_method_cd3d46d0().update(string);
        });
    }
