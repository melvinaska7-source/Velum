...
public class SnowballManager implements iIIiIIiIi_Class294 {
   ...
   private static final Identifier BLOOM_TEXTURE = VelumClient.id("textures/bloom.png");
   ...
   public final IiIIIiII_Class69<KeyPressEvent> onKeyPress = var1 -> {
      if (var1.getAction() == 1) {
         if (I_field_3a9bda27.currentScreen == null) {
            GlobalsMenuModule var2 = VelumClient.getInstance().getModuleManager().getModule(GlobalsMenuModule.class);
   ...
   public void throwSnowball() {
      if (I_field_3a9bda27.player != null && timer.I_method_58432069(500L)) {
         ...
         Packets.Nested1_242f4d2b var3 = VelumClient.getInstance().I_method_cd3d46d0().getGameInfo();
         if (var3 != null) {
   ...
   public void tick() {
      ...
      if (I_field_3a9bda27.player instanceof FakeFrozenTicksAccess var7) {
         int var9 = var7.velum$getFakeFrozenTicks();
         if (this.targetFrozenTicks > 0) {
            if (var9 < this.targetFrozenTicks) {
               var7.velum$setFakeFrozenTicks(Math.min(var9 + 20, this.targetFrozenTicks));
            } else {
               this.targetFrozenTicks = 0;
            }
         } else if (var9 > 0) {
            var7.velum$setFakeFrozenTicks(var9 - 2);
         }
      }
   }
