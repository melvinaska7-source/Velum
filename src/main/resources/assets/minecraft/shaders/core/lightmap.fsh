...
uniform vec3 VelumNightTint;
uniform float VelumNightStrength;
...
void main() {
    ...
    if (VelumNightStrength > 0.0) {
        ...
        if (!guiCorner) {
            ...
            vec3 night = mix(VelumNightTint, vec3(1.0), desat) * lit;
            color = mix(color, night, VelumNightStrength);
        }
    }
