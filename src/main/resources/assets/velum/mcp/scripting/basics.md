# Скрипты для чит-клиента Velum (Minecraft)

## Что это за система

- **Язык — Python 3** (исполняется через Jep внутри клиента). **Lua в клиенте НЕТ** (папка `docs/lua-scripts` — легаси-название, не обращай на него внимания). Java-API скриптов лежит в пакете `pyvelum`.
...
- **Глобалы инжектятся автоматически. Их НЕЛЬЗЯ и НЕ НУЖНО импортировать** — никакого `from velum import ...`. `Module`, `events`, `client`, `mc`, `hud`, `world` и прочее просто есть в неймспейсе.
...
def on_render(event):
    ...
    ctx.drawText(font("medium", 8), "Velum", 98, 98, white)
...
| `client` | `msg/warn/error/overlay`, `find(...)`, `modules`, `gameDir()` (= папка клиента `.minecraft/Velum`), `menu_opened()`, `cursor(...)`, `font_width/height` |
