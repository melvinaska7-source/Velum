...
def draw(d):
    ...
    if show_fps.get():
        ...
        d.text("Velum", 7, 16, size=6, color=GRAY)
...
def menu(ui):
    ...
    with ui.row(gap=0, radius=10, bg=WIN, fill=True):
        ...
        with ui.column(gap=3, pad=(10, 8), width=90):
            ui.text("Velum", size=8, weight="bold", color=ACCENT)
            ui.space(4)
            for c in present:
...
`aura.module` и `ctx.handler` — обёртки над классами КЛИЕНТА (`moscow.velum.*`): мост зовёт их методы по именам как есть.
