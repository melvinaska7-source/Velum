# Velum Script API — полный справочник

Источник правды по коду клиента: `prelude.py` (формирует неймспейс), Java-классы `pyvelum/**`, загрузчик `systems/python/**`, команда `.py`.
...
- **Классы вне маппингов** (наши события, `moscow.velum.*`, java.*, brigadier, authlib, joml) мост зовёт по имени **как есть** — это рабочий путь, но имя должно совпадать буква в букву. Для имени сущности всё равно надёжнее `getNameForScoreboard()`, чем `getName().getString()`.
...
`isinstance(...)` и `obj.getClass().getName()` в релизе молча не сработают. `is_instance` ловит и наследников (`$Full`, `$LookAndOnGround`, ...). Старый вызов `McBridge.isInstance(obj, fqn)` (`from pyvelum import McBridge`) тоже работает.
...
assets.resource("gui/icon.png")         # ТОЛЬКО ресурсы клиента (assets/velum/...)
...
paths.root()        # <gameDir> = .minecraft/Velum (папка клиента)
...
> `assets.resource(path)` достаёт **только ресурсы клиента** (`assets/velum/…`). Ванильные текстуры Minecraft через него не получить — для них нужен `Identifier` (`jimport("net.minecraft.util.Identifier")`).
...
client.gameDir()          # папка КЛИЕНТА: .minecraft/Velum (== paths.root()), а НЕ папка игры
...
profile.client; profile.version   # "Velum", "2.1"
...
Цели: `players` (виды `local`, `friends`, `velum_users`, `others`), `mobs`, `animals`, `items`.
...
> `ctx.target` может быть `None` — проверяй. `ctx.handler` и `aura.module` — обёртки над классами КЛИЕНТА (`moscow.velum.*`): мост зовёт их методы по именам как есть.
