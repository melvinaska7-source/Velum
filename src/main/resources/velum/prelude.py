...
from pyvelum.utility.render import ColorRGBA, PyAssets, PyRender3D, PyShaders
from pyvelum.ui import PyScreen
...
if not getattr(sys, "__velum_sandbox__", False):
    _sandbox_install()
    sys.__velum_sandbox__ = True
...
def __velum_make_ns(_events, _client, _mc, _gv, _py_aura=None, _py_rotations=None):
    if _py_aura is None:
        from pyvelum.classes.aura import PyAura
        _py_aura = PyAura()
    if _py_rotations is None:
        from pyvelum.classes import PyRotations
